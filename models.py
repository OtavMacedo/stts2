...
def load_checkpoint1(model, optimizer, path, load_only_params=True, ignore_modules=[]):
    state = torch.load(path, map_location="cpu")
    params = state["net"]
    ...
    for key in model:
        if key in params and key not in ignore_modules:
            ...
            try:
                ...
                print(
                    f"{key} key length: {len(model[key].state_dict().keys())}, state_dict length: {len(state_dict.keys())}"
                )
